...
const validColumns = headers.map((_, i) => rows.some((row) => row[i] !== null && row[i] !== ''))
```
...
```typescript
const requiredColumns = columnConfigs.filter((c) => c.category === 'required').map((c) => c.name)
